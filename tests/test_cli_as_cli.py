...
""")
            ...
            with patch("as_docs.engine.run_generate") as mock_gen:
                ...
                runner.invoke(generate, ["--use-as-cli", "--no-ai"])
            ...
            with patch("as_docs.engine.run_generate") as mock_gen:
                ...
                mock_graph._regen_meta = {
                    "as_cli_merge_report": {
                        ...
                        "pou_count_merged": 5,
                        "conflicts": [
                            {"pou_name": "Main", "conflict_type": "path_mismatch"}
                        ],
                    }
                }
                mock_gen.return_value = mock_graph
...
""")
            ...
            with patch.object(as_docs.cli, "AsCliAdapter") as MockAdapter:
                ...
                mock_adapter.get_logical_list.return_value = {
                    "modules": [{"name": "Main"}]
                }
...
""")
            ...
            with patch.object(as_docs.cli, "AsCliAdapter") as MockAdapter:
                ...
                assert result.exit_code == 0
                assert (
                    "Running in graceful fallback mode (recommended)" in result.output
                )
