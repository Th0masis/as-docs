...
def enrich_graph(graph: KnowledgeGraph, level: int, config: Config, project_root: Path | None = None) -> CacheStats:
    ...
    if level >= 3:
        _enrich_pous(graph=graph, config=config, cache=cache, provider=provider, stats=stats, project_root=project_root)
...
def _write_pou_hashes(graph: KnowledgeGraph, config: Config, project_root: Path | None = None) -> None:
